...
(specification "Base translation -- tr"
  (assertions
    ...
    (tr 4) =fn=> (fn [s] (re-matches #".*on line [1-9][0-9]* in com.fulcro.*" s))))
...
(specification "Message translations with context"
  (assertions
    ...
    (trc 1 4) =fn=> (fn [s] (re-matches #".*on line [1-9][0-9]* in com.fulcro.*" s))
  ...
  (i18n/with-locale deflt-format locale-with-no-translations
    ...
    (let [s "str"]
      (assertions
        ...
        (trf s) =fn=> (fn [s] (re-matches #".*on line [1-9][0-9]* in com.fulcro.*" s))))
    (assertions
...
#?(:clj
   (specification "Locale override in SSR"
     (let [initial-tree     (comp/get-initial-state Root {})
           es-locale        (i18n/load-locale "fulcro" :es)
